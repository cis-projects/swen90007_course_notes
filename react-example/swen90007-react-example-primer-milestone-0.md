...
An error is reported, why? Inspect the network calls and notice that the request to the `/test` resource has been blocked because the *CORS header 'Access-Control-Allow-Origin' \[is\] missing*. CORS stands for Cross Origin Resource Sharing, and is a security feature of all modern web browsers. What happening here is that our browser is refusing to honor a request to the API, because it believes that request to be unsafe.
